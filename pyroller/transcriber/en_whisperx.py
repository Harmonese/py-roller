...
class EnglishWhisperXTranscriber(Transcriber):
    def __init__(
        self,
    ...
    def transcribe(self, audio_artifact: AudioArtifact, language: str, tone_mode: str, progress: ProgressReporter | None = None) -> TranscriptionResult:
        ...
        logger.info("Loading WhisperX English model=%s device=%s", self.model_name, self.device)
        if stage is not None:
        ...
        logger.info("Loading audio for English transcription: %s", audio_artifact.path)
        if stage is not None:
        ...
        if self.align_words:
            try:
                logger.info("Running WhisperX English word alignment")
                if stage is not None:
                ...
                logger.warning("WhisperX English word alignment failed; falling back to segment timestamps: %s", exc)
                aligned = False

        if stage is not None and not self.align_words:
            stage.phase("post-processing transcription")
        units = self._segments_to_arpabet_units(segments, language=language)
        raw_text = " ".join(seg.get("text", "") for seg in segments).strip() or None

        del model
        gc.collect()

        logger.info("English WhisperX transcription complete: %d segments, %d ARPAbet timed units", len(segments), len(units))
        if stage is not None:
    ...
    def _segments_to_arpabet_units(self, segments: list[dict[str, Any]], language: str) -> list[TimedUnit]:

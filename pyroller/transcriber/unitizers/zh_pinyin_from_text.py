...
class ZhPinyinFromTextUnitizer(TranscriptionAdapter):
    ...
    def _unitize(self, engine_output: EngineOutput, *, language: str, tone_mode: str) -> list[TimedUnit]:
        ...
        for span in preferred_text_spans(engine_output):
            units.extend(self._text_span_to_units(span, language=language, tone_mode=tone_mode))
        return units

    def _text_span_to_units(self, span: EngineSpan, *, language: str, tone_mode: str) -> list[TimedUnit]:
        text = span.text or ""
        normalized = normalize_chinese_text(text)
        syllables = chinese_text_to_pinyin_syllables(text, tone_mode=tone_mode)
        if not syllables:
            return []

...
def _escape_ass_text(text: str) -> str:
    """Escape ASS control characters so literal braces and backslashes
    are not interpreted as override tags or escape sequences."""
...
class ASSKaraokeWriter(Writer):
    ...
    def write(self, alignment: AlignmentResult, output_path: Path) -> WriteResult:
        ...
        with output_path.open("w", encoding="utf-8") as f:
            f.write(_ASS_HEADER_TEMPLATE.format(generated_by=_("; Generated by {by_tag}").format(by_tag=self.by_tag)))
        ...
        logger.info(_("Wrote %d ASS karaoke lines to %s"), written_lines, output_path)
        return WriteResult(
    ...
    def _line_to_ass_text(self, line: AlignmentLine) -> str:
        if not line.aligned_units:
            return _escape_ass_text(line.raw_text)
        chunks: list[str] = []
        for unit in line.aligned_units:
            duration_cs = seconds_to_centiseconds(max(unit.end_time - unit.start_time, 0.01))
            if duration_cs <= 0:
                duration_cs = 1
            chunks.append(f"{{\\{self.tag_type}{duration_cs}}}{_escape_ass_text(unit.text)}")
        return "".join(chunks)
